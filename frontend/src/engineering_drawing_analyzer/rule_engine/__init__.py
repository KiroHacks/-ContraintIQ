...
<<<<<<< HEAD
    ...
    "CompositeFCFRule",
    "DatumFeatureSymbolPlacementRule",
<<<<<<< HEAD:frontend/src/engineering_drawing_analyzer/rule_engine/__init__.py
    "TitleBlockRule",
    "SurfaceFinishRule",
    "HoleSpecificationRule",
    "ViewSufficiencyRule",
...
>>>>>>> 1b37df4b6b3322109ca88ea01d08d609f959330f:src/engineering_drawing_analyzer/rule_engine/__init__.py
]
...
def build_verification_report(
    ...
    # overall_status: "Pass" iff zero issues
    overall_status = "Pass" if len(issues) == 0 else "Fail"

    # issue_counts: count per Severity level
    severity_counter: Counter[str] = Counter()
    for sev in Severity:
        severity_counter[sev.value] = 0  # ensure all keys present
    for issue in issues:
        severity_counter[issue.severity.value] += 1
    issue_counts: dict[str, int] = dict(severity_counter)

    # systemic_patterns: when more than 3 issues share the same issue_type
    ...
    for issue_type, count in type_counter.items():
        if count > _SYSTEMIC_PATTERN_THRESHOLD:
            # Find a standard_reference from one of the issues of this type
            standard_ref = None
            for issue in issues:
